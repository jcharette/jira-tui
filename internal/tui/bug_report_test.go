...
func TestBugReportToggleDiagnosticsOnlyOnCheckbox(t *testing.T) {
	model := NewModel(&fakeIssueSearcher{}, "project = ABC")
	defer model.workers.Stop()
	...
	if !model.bugReportIncludeDiagnostics {
		t.Fatal("expected diagnostics to default on when events exist")
	}
	...
	next = updated.(Model)
	if next.bugReportIncludeDiagnostics {
		t.Fatal("space on checkbox should toggle diagnostics off")
	}
}
