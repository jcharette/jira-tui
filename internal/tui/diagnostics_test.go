...
func TestPersistentDiagnosticLogRedactsSensitiveKeyValues(t *testing.T) {
	path := filepath.Join(t.TempDir(), "diagnostics.jsonl")
	log, err := OpenPersistentDiagnosticLog(path)
	if err != nil {
		t.Fatalf("OpenPersistentDiagnosticLog() error = %v", err)
	...
	log.RecordDiagnosticEvent(diagnosticEvent{
		At:     time.Date(2026, 6, 18, 20, 48, 0, 0, time.UTC),
		Kind:   diagnosticKindState,
		Label:  "unsafe",
		Status: "error",
		Detail: "token=secret-token password=hunter2 api_token=abc123 safe=value",
	})
	if err := log.Close(); err != nil {
		t.Fatalf("Close() error = %v", err)
	}
	...
	for _, leak := range []string{"secret-token", "hunter2", "abc123"} {
		if strings.Contains(record["detail"], leak) {
			t.Fatalf("persistent diagnostics leaked %q in %q", leak, record["detail"])
		}
	}
	for _, want := range []string{"token=[redacted]", "password=[redacted]", "api_token=[redacted]", "safe=value"} {
		if !strings.Contains(record["detail"], want) {
			t.Fatalf("persistent diagnostics detail = %q, missing %q", record["detail"], want)
		}
	}
}
