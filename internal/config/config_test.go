...
`)
	...
	if !cfg.Notifications.Enabled || !cfg.Notifications.SystemEnabled || !cfg.Notifications.SystemOnNew || !cfg.Notifications.SystemOnUpdates || !cfg.Notifications.AutoOpenPanel || !cfg.Notifications.KeepPanelOpenUntilCleared {
		t.Fatalf("Notifications = %#v", cfg.Notifications)
	}
	if cfg.Notifications.MaxItems != 75 {
		t.Fatalf("Notifications.MaxItems = %d", cfg.Notifications.MaxItems)
	}
...
func TestLoadRejectsHTTPBaseURL(t *testing.T) {
	path := writeConfig(t, `
version = 1
active_profile = "default"
...
`)
	...
	if err == nil {
		t.Fatal("expected HTTP base URL validation error")
	}
	if !strings.Contains(err.Error(), "https://") {
		t.Fatalf("error = %v, want HTTPS guidance", err)
	}
}
...
func TestSaveWritesConfigFileWithPrivatePermissions(t *testing.T) {
	...
	cfg.Notifications.SystemEnabled = true
	cfg.Notifications.SystemOnNew = true
	cfg.Notifications.SystemOnUpdates = true
	cfg.Notifications.AutoOpenPanel = true
	cfg.Notifications.KeepPanelOpenUntilCleared = true
	cfg.Notifications.MaxItems = 75
	...
	for _, want := range []string{
		...
		if !strings.Contains(string(data), want) {
			t.Fatalf("saved config missing %q: %s", want, string(data))
		}
	}
...
func TestDefaultsEnablePersistentInAppNotifications(t *testing.T) {
	cfg := Defaults()

	if !cfg.Notifications.Enabled {
		t.Fatal("notifications should default to enabled")
	}
	if !cfg.Notifications.AutoOpenPanel {
		t.Fatal("notification panel should auto-open by default")
	}
	if !cfg.Notifications.KeepPanelOpenUntilCleared {
		t.Fatal("notification panel should stay open until cleared by default")
	}
	if !cfg.Notifications.SystemOnNew {
		t.Fatal("system notification policy should include new tickets by default")
	}
	if cfg.Notifications.SystemEnabled {
		t.Fatal("system notifications should require opt-in")
	}
	if cfg.Notifications.SystemOnUpdates {
		t.Fatal("system update notifications should require opt-in")
	}
	if cfg.Notifications.MaxItems != 50 {
		t.Fatalf("Notifications.MaxItems = %d, want 50", cfg.Notifications.MaxItems)
	}
}
