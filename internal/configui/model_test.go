...
func TestRenderShowsNotificationsSection(t *testing.T) {
	cfg := config.Defaults()
	...
	model.width = 120
	model.height = 30
	model.section = sectionNotifications

	view := model.render()

	for _, want := range []string{"Notifications", "Enabled", "System Enabled", "System On Updates", "Auto Open Panel", "Keep Panel Open", "Max Items"} {
		if !strings.Contains(view, want) {
			t.Fatalf("missing %q in %q", want, view)
		}
	}
...
func TestConfigFromFieldsIncludesNotificationSettings(t *testing.T) {
	cfg := config.Defaults()
	cfg.ActiveProfile = "default"
	cfg.BaseURL = "https://example.atlassian.net"
	cfg.Email = "person@example.com"
	...
	cfg.ActiveView = cfg.Views[0].Name
	model := NewModel("/tmp/jira.toml", cfg, nil)
	setFieldForTest(&model, "System Enabled", "true")
	setFieldForTest(&model, "System On Updates", "true")
	setFieldForTest(&model, "Max Items", "75")
	...
	if !next.Notifications.Enabled || !next.Notifications.SystemEnabled || !next.Notifications.SystemOnNew || !next.Notifications.SystemOnUpdates || !next.Notifications.AutoOpenPanel || !next.Notifications.KeepPanelOpenUntilCleared {
		t.Fatalf("Notifications = %#v", next.Notifications)
	}
	if next.Notifications.MaxItems != 75 {
		t.Fatalf("Notifications.MaxItems = %d", next.Notifications.MaxItems)
	}
}
...
func TestPickerFieldsDoNotTrapSectionNavigation(t *testing.T) {
	...
	next = updated.(Model)
	if next.section != sectionNotifications {
		t.Fatalf("section after tab from boolean field = %d, want Notifications", next.section)
	}
	if value := fieldValueForTest(next, "Enabled"); value != "false" {
